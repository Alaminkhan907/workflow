...
// 20 secs
jest.setTimeout(20000); // Set timeout for all tests in this file
...
describe('POST /signup', () => {
  ...
  it('should log in an existing user and return a token', async () => {
    ...
    const user = await User.create({
      ...
      password: 'password123', // Save the hashed password
      role: 'worker',
    });
    ...
    const res = await request(app)
      ...
      .send({
        email: 'jane@example.com',
        password: 'password123', // Plain text password to match
      });
